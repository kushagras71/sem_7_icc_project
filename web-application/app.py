    ...
    for root, dirs, files in os.walk("./media/public-keys/"):
        for file in files:
            ...
            if list[0] == username:
                ...
                return send_file(
                    filename, attachment_filename="publicKey.pem", as_attachment=True
                )
...
def download_file(filename):
    ...
    if os.path.isfile(filepath):
        return send_file(
            filepath,
            attachment_filename="fileMessage-thrainSecurity.txt",
            as_attachment=True,
        )
    else:
        return render_template(
            "file-list.html", msg="An issue encountered, our team is working on that"
        )
...
def downloads_pk():
    ...
    if len(username) == 0:
        return render_template(
            "public-key-list.html", msg="Aww snap! No public key found in the database"
        )
    else:
        return render_template(
...
def download_f():
    for root, dirs, files in os.walk(UPLOAD_FOLDER):
        if len(files) == 0:
            return render_template(
                "file-list.html", msg="Aww snap! No file found in directory"
            )
        else:
            return render_template(
                "file-list.html", msg="", itr=0, length=len(files), list=files
            )
